...
def send_welcome_email(email):
    ...
    aws = json.load(open(os.path.join(script_dir, "aws-keys.txt"),"r"))
    conn = boto.ses.connect_to_region('us-east-1', aws_access_key_id=aws['access'], aws_secret_access_key=aws['secret'])
    conn.send_email('SyncDrive <synergy.sjsu@gmail.com>', 'Welcome to SyncDrive!', 'Welcome to SyncDrive, a one stop shop to manage all your cloud files. You can login to www.sync-drive.net:5000 with your registered password to manage all your cloud files in SyncDrive.', [email])
